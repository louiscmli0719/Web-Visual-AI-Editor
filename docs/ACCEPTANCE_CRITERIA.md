...
3. V0.4 标尺测距、V0.5 共享元素与 V0.5.5 UI Refresh 已完成代码实现；V0.6 自动布局辅助、V0.7 字体读取/切换与 V0.8 直接操作基础层已完成代码接入。当前自动化已覆盖 96 个单元测试与 TypeScript；Chrome 真实插件图标点击仍待手动验收。
...
Given 当前页面处于编辑模式
When 用户选择一个有父容器的元素
Then Panel 展示父容器基础布局信息

通过标准：

1. “布局辅助”区块显示父容器 selector、display、方向、对齐、gap 和当前子元素位置。
2. 未选中元素时显示空状态。
3. 用户能保存布局方向、对齐方式、目标间距和补充说明。
4. 保存后记录列表新增一条布局记录。
5. 导出 JSON 包含当前版本号、`layoutContext` 和 `layoutIntent`；当前 V0.8 构建版本号为 `0.8`。
6. 复制 Prompt 时包含“布局辅助”段。
7. 旧版 JSON 导入后布局字段降级为 `null`，不影响旧记录展示。
...
5. 自动布局拖动时，紫色拖手、参考线、目标框和位置标签必须易于辨认。
6. Hover、selected、measurement、auto-layout 状态必须易区分。
7. 编辑模式状态必须明显。
8. 退出编辑模式后，宿主页面交互和本次拖动造成的 DOM 顺序恢复正常。
9. 导入失败、复制失败、无选中元素、空记录列表、本地字体读取失败都有明确提示。
...
1. 当前 V0.8 已通过 `cd extension && npm run verify`，覆盖 96 个单元测试、TypeScript 类型检查和 Vite 构建。
...
3. 本次 Chrome 烟测覆盖 Panel / Overlay 创建、真实页面元素点击、V0.4 A/B 双元素测距、V0.5 相似按钮批量记录、记录列表展示、JSON 导出、Panel 标题栏拖动、删除弹窗焦点循环和 Prompt 复制。
...
当前状态：代码实现完成，已通过单元测试、TypeScript、构建验证与 Chrome 148 pipe 自动烟测，等待 Chrome 手动视觉与交互复核。
...
当前状态：代码整合完成，当前 V0.8 构建已通过 96 个单元测试、TypeScript、Vite build；V0.5.5 主流程已通过 Chrome 148 pipe 自动烟测，顶部工具栏已按 Figma「绘管家」节点对齐，图标已通过 Vibma 从 Figma 节点导出接入，等待 Chrome 手动视觉与交互复核。
...
6. 工具栏外观与 Figma 顶栏一致：黑色胶囊背景、38px 图标按钮、Figma 导出图标、圆形 hover / active 态、记录按钮文字和蓝色数量徽标；导入、导出 JSON 与复制 Prompt 仍保留在 Panel 交付区。
...
2. 打开 `extension/test-pages/basic.html`，点击插件图标，检查浮动工具栏、暗色面板和空状态卡片；工具栏需对齐 Figma 顶栏的按钮顺序、胶囊质感、Figma 图标、圆形 hover / active 态与记录蓝色徽标。
