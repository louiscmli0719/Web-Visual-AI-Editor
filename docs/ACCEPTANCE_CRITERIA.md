...
3. V0.4 标尺测距、V0.5 共享元素与 V0.5.5 UI Refresh 已完成代码实现；当前 `npm run verify` 覆盖 89 个单元测试、TypeScript 与 Vite build，等待 Chrome 联合手动验收。
...
3. 面板标题栏可拖动，用户能把面板移开正在检查的页面区域。
4. 长度类数值输入采用左数值右单位的胶囊样式，默认 `px`，支持 `pt`，清空后回到 `0`。
5. Hover 和 selected 状态必须易区分。
6. 编辑模式状态必须明显。
7. 退出编辑模式后，宿主页面交互恢复正常。
8. 导入失败、复制失败、无选中元素、空记录列表都有明确提示。
...
当前状态：代码实现已完成；当前 V0.5 构建已覆盖 V0.4 能力，等待 Chrome 手动加载复核。
...
Then Overlay 显示 A（红粉虚线）和 B（橙红虚线）高亮框，以及距离线和数值标签

通过标准：

1. 进入测距模式时 Panel 显示"请点击页面元素选择 A"。
2. 选完 A 后 Panel 显示"请点击页面元素选择 B"。
3. 选完 B 后 Overlay 显示水平/垂直距离线，Panel 显示水平/垂直/中心距离。
4. 提供"重新测距"和"退出测距模式"按钮。
5. 测距模式下点击同一个元素时显示提示。
...
1. 当前 V0.5 构建导出 JSON 的 `version: "0.5"`，单元素测距记录的 `sharedGroup` 为 `null`。
...
8. 导出 JSON → 检查 `version: "0.5"`、`measurements` 字段，以及单元素记录的 `sharedGroup: null`。
...
当前状态：代码实现完成，已通过单元测试、TypeScript 与构建验证，等待 Chrome 手动加载复核。
...
1. 新导出的 JSON 为 `version: "0.5"`，批量记录包含 `sharedGroup.matchLevel/primaryFeature/totalMatched/truncated/targets`。
2. 导入 V0.1 至 V0.4 数据时，各记录补 `sharedGroup: null`。
3. 导入 V0.5 批量记录时，保存的 `sharedGroup` 原样保留，不重新识别页面。
...
当前状态：代码整合完成，已通过 89 个单元测试、TypeScript 与 Vite build，等待 Chrome 手动加载复核。

### 10.1 视觉与布局

1. 浮动工具栏固定在页面顶部，选择、测距、记录与关闭操作均可用。
2. Panel 使用 Sketch-style 哑光深色背景、深灰胶囊控件和蓝紫强调色，宽度最大为 360px。
3. Panel 默认位于右侧，拖动标题栏后可自由移动，并在视口内保留可操作区域。
4. 元素信息、样式编辑、测距、相似元素、评论、记录行和交付区均保持明确的视觉层级。
5. 选中元素时 Overlay 显示红粉选中框与包含 tag/尺寸的深色标签；测距线和相似目标均使用红粉高对比反馈。
...
4. 使用键盘可操作批量范围徽标、“高亮全部”、属性/记录导航和工具栏按钮。
...
2. 打开 `extension/test-pages/basic.html`，点击插件图标，检查浮动工具栏、暗色面板和空状态卡片。
3. 拖动 Panel 标题栏到页面左侧、底部和右侧边缘，确认拖动顺畅且不会越出视口；调整窗口尺寸后确认 Panel 仍可见。
4. 选中普通元素，检查元素尺寸标签、样式卡片、页面评论与元素评论各自控件 label 聚焦关系。
5. 在样式编辑中选中一个长度输入，确认数值框与单位胶囊分离，默认 `px`，切换到 `pt` 后自动换算，清空后变为 `0`。
6. 完成一次 V0.4 双元素测距与一次 V0.5 批量范围保存，检查 Overlay 色彩、记录行块、蓝紫徽标与交付区。
7. 新增并编辑页面评论，确认元素评论保存入口在编辑期间不出现，导出数据仍满足第 9 节页面级字段约束。
8. 删除一条记录，使用键盘验证取消焦点、焦点循环、`Escape` 关闭与焦点恢复。
9. 切换到系统“减少动态效果”后复核主要交互，再退出编辑模式确认宿主页面无残留。
...
测试页面用于确认插件能覆盖常见 DOM 结构、交互态、测距与批量范围场景。
